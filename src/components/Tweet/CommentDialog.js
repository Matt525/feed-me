...
export default function CommentDialog({activity, onPostComment, onClickOutside}){ 
    { const { object: {data: tweet}, } = activity;
const tweetActor = activity.actor;
// format the tweet text into a separate function so it can be reused across the codebase.
const formatTweetText = (text) => {
    return formatStringWithLink(text, 'tweet__text--link', true)
    .replace(/\n/g, '<br/>');
}
